...
// PUT YOUR CODE HERE
			...
			// Calling resize1D to work its magic.
			resize1D(src, w, ww, 1, kernelID, param, temp);

			// changing channel to a new length of the respective images
			src += w;
			temp += ww;
		}

		// Resetting temp pointer back to the beginning
		IP_getChannel(I3, ch, temp, t);
...
// PUT YOUR CODE HERE
			...
			// Since were working on the columns, we are just incrementing
			// the channels by 1 to move to the new column
			resize1D(temp++, h, hh, ww, kernelID, param, dst++);
